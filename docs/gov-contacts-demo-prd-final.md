# 政务通讯录管理后台需求文档（PRD）- 增强版

## 1. 新手导读

### 1.1 产品一句话
`gov-contacts-demo` 是一个面向政务组织管理场景的通讯录与组织协同后台，覆盖组织架构、人员任职、审批流、条线管理、可见性规则与展示配置，为政务组织提供统一的人员信息治理平台。

### 1.2 为什么做
- 政务组织层级深、跨部门协作频繁，传统通讯录无法承载"行政区划+条线+任职"复杂关系。
- "谁可以看谁、看哪些字段"的规则复杂，缺少统一配置与验证能力。
- 人员转调、外部协作接入等高频变更场景需要流程化与可追踪。
- 现有系统数据分散，缺乏统一的治理入口和流程闭环。

### 1.3 核心价值
- **统一治理**：组织、人员、条线、权限规则在同一平台管理，实现数据一致性。
- **流程闭环**：转调审批 -> 生效执行 -> 数据同步 -> 规则重算，减少人工干预。
- **可控可验**：规则配置可验证、关键动作可追踪、结果可验收，提高系统可靠性。
- **灵活扩展**：支持自定义字段、展示配置，适应不同政务组织的个性化需求。

### 1.4 阅读建议
- 首次阅读：先看第 2、3、4 章建立整体认知。
- 研发测试：重点看第 5、6、7、8、9、10 章。
- 运营复盘：重点看第 13 章埋点方案与指标口径。
- 数据建模：重点看第 12 章数据结构与字段字典。
...
- 随着政务数字化转型的深入，对人员信息管理的安全性、准确性、实时性要求日益提高。

#### 2.1.2 需求价值
- **管理价值**：统一组织、人员、条线、规则配置入口，降低治理复杂度，提高管理效率。
- **业务价值**：建立"发起-审批-生效-重算"闭环流程，减少人工干预和流程断点，确保数据及时更新。
- **产品价值**：沉淀政务场景标准能力模型，支持后续跨项目快速复制，形成行业解决方案。
- **安全价值**：通过细粒度的可见性规则和权限控制，保障敏感信息安全，符合政务数据安全要求。

### 2.2 文档目标
明确当前版本需求边界、功能设计、交互规则、异常策略、验收标准和数据追踪方案，确保产品、设计、研发、测试、运营对交付标准一致，为系统开发和验收提供明确依据。

### 2.3 适用范围
- 前端后台系统：Vue3 + Element Plus + Vue Router。
- 数据层：Mock Provider（本地存储），预留 Real Provider 结构。
- 范围覆盖：组织/人员、审批中心、外部人员、条线/区划、可见性、展示配置、自定义字段。
- 本期不包括：真实后端接口开发、大规模数据性能优化、生产级监控系统集成。
...
- **转调**：人员在不同部门或条线之间的调动。
- **外部人员**：非本组织正式编制，但需要临时访问系统的人员。
- **审批单**：记录人员转调、外部人员接入等流程的单据。
- **待生效队列**：存放定时生效任务的队列。
...
- 打通"信息维护-审批变更-权限重算"的闭环链路，确保数据一致性。
- 支持外部协作与临时组织，提升复杂协同场景承载能力。
- 提供灵活的配置能力，满足不同政务组织的个性化需求。
- 建立完善的埋点体系，为运营和优化提供数据支撑。

### 3.2 用户目标
- 管理员可低成本完成组织信息治理与变更控制，提高管理效率。
- 普通用户可基于权限稳定查看准确联系人信息，方便日常工作。
- 审批角色可高效处理变更流程，减少线下沟通成本，确保流程规范。
- 系统使用简单直观，降低学习成本，提高用户满意度。

### 3.3 角色分层
- `platform_admin`：平台级全局管理权限，可访问所有功能模块，进行全局配置。
- `org_admin`：组织级管理权限，可管理本组织及其下属组织的人员和审批。
- `user`：基础查看权限（无法执行管理动作），可查看授权范围内的人员信息。
...
- 页面级功能点统计：每个页面的核心功能单独计点。

### 4.2 模块功能点清单（详细）

> 说明：首页 `ProjectHome` 仅用于汇报展示，不纳入功能点统计与排期评估。

#### 4.2.1 组织架构与人员管理（35 点）
| 功能点 | 类型 | 优先级 | 计点 |
|---|---|---|---|
| 组织树查询/搜索 | 查询 | P0 | 1 |
| 组织节点新增 | 新增 | P0 | 1 |
...
|---|---|---|---|
| 临时组织列表查询 | 查询 | P1 | 1 |
| 临时组织新增/编辑 | 新增/编辑 | P1 | 1 |
| 到期提醒计算 | 流程 | P1 | 1 |
| 提醒状态刷新 | 编辑 | P1 | 1 |
| 提醒结果展示 | 查询 | P1 | 1 |
| 提醒通知设置 | 配置 | P2 | 1 |

#### 4.2.5 条线通讯录与区划/编码管理（22 点）
| 功能点 | 类型 | 优先级 | 计点 |
|---|---|---|---|
| 行政区划树查询 | 查询 | P0 | 1 |
| 行政区划节点新增 | 新增 | P0 | 1 |
...
- 优先级策略：P0 功能必须在首版实现，P1 功能在首版或次版实现，P2 功能可在后续版本实现。
...
- **当前UI样式**：使用Element Plus组件库，左侧组织树，右侧人员列表，顶部操作按钮栏，筛选条件栏，分页控件。
- **目标UI样式**：保持Element Plus风格，优化交互体验，增加操作反馈，提升视觉一致性。

### 5.2 人员编辑页 `PersonForm`
- **功能**：人员新增/编辑、任职管理、秘书关系管理、草稿恢复与离开保护。
- **字段**：
  - 基础信息：姓名、手机、邮箱、部门、用户名、登录方式、民族、工号、党派、身份证、入职日期、编制、职级、政治面貌、首次工作时间、家庭地址、短号。
  - 任职信息：部门、职务、人员角色、直属主管、职位传真、办公电话、办公地址、职务层次、任职类型、开始日期、结束日期。
  - 秘书关系：领导姓名、秘书姓名、关系类型、开始日期、结束日期。
- **按钮**：保存、取消、添加任职、编辑任职、删除任职、添加秘书关系、编辑秘书关系、删除秘书关系。
- **交互**：姓名必填；任职至少一条；离开页面弹确认；自动保存草稿；保存后回列表。
- **当前UI样式**：使用Element Plus组件库，表单布局，分区域展示基础信息、任职信息、秘书关系。
- **目标UI样式**：保持Element Plus风格，优化表单布局，增加字段验证反馈，提升用户体验。
...
- **当前UI样式**：使用Element Plus组件库，详情页布局，分区域展示人员信息、任职信息、秘书关系。
- **目标UI样式**：保持Element Plus风格，优化信息展示布局，增加信息分类，提升可读性。
...
- **当前UI样式**：使用Element Plus组件库，审批单列表，流程配置表单，待生效队列列表。
- **目标UI样式**：保持Element Plus风格，优化审批流程展示，增加状态可视化，提升操作体验。
...
- **当前UI样式**：使用Element Plus组件库，外部人员列表，申请表单，审批操作按钮。
- **目标UI样式**：保持Element Plus风格，优化状态展示，增加到期提醒视觉效果，提升管理效率。
...
- **当前UI样式**：使用Element Plus组件库，临时组织列表，提醒状态展示。
- **目标UI样式**：保持Element Plus风格，优化提醒状态视觉效果，增加到期倒计时，提升提醒效果。
...
- **当前UI样式**：使用Element Plus组件库，树形结构展示行政区划和条线，右侧操作面板。
- **目标UI样式**：保持Element Plus风格，优化树形结构展示，增加节点操作便捷性，提升管理效率。
...
- **当前UI样式**：使用Element Plus组件库，左侧条线树，右侧成员列表和配置面板。
- **目标UI样式**：保持Element Plus风格，优化成员管理界面，增加排序操作便捷性，提升管理效率。
...
- **当前UI样式**：使用Element Plus组件库，规则列表，规则编辑表单，验证结果展示。
- **目标UI样式**：保持Element Plus风格，优化规则配置界面，增加规则验证可视化，提升配置效率。
...
- **当前UI样式**：使用Element Plus组件库，配置表单，预览区域，分组管理界面。
- **目标UI样式**：保持Element Plus风格，优化配置界面，增加实时预览功能，提升配置体验。
...
- **当前UI样式**：使用Element Plus组件库，字段列表，字段编辑表单。
- **目标UI样式**：保持Element Plus风格，优化字段管理界面，增加字段类型可视化，提升配置效率。
...
- **当前UI样式**：使用Element Plus组件库，编码列表，编码编辑表单。
- **目标UI样式**：保持Element Plus风格，优化编码管理界面，增加编码层级可视化，提升管理效率。
...
- **按钮**：返回首页、返回通讯录。
- **交互**：点击后跳转到对应安全页面。
- **当前UI样式**：使用Element Plus组件库，错误提示页面，导航按钮。
- **目标UI样式**：保持Element Plus风格，优化错误提示界面，增加导航选项，提升用户体验。
...
- **当前UI样式**：模拟移动端名片样式，展示不同版本的设计效果。
- **目标UI样式**：优化移动端名片展示效果，增加交互演示，提升视觉体验。
...
```mermaid
flowchart TD
  A[进入人员编辑页] --> B[查看现有秘书关系]
  B --> C{操作类型}
  C -->|添加秘书关系| D[填写领导和秘书信息]
  C -->|编辑秘书关系| E[修改关系信息]
  C -->|删除秘书关系| F[确认删除]
  D --> G[验证关系信息]
  E --> G
...
### 6.9 人员导入流程
1. 管理员进入人员管理页面，点击导入按钮。
2. 选择导入策略（新增/更新/全量覆盖）。
3. 上传Excel文件。
4. 系统进行预检，显示导入结果（成功/失败数量）。
5. 若有失败，下载错误明细并修正。
6. 确认后开始导入。
7. 系统执行导入并显示最终结果。
...
- 列表空数据：统一空态组件反馈。
- 数据加载失败：显示错误提示并支持重试。
- 数据保存失败：显示错误原因并支持重新提交。

### 8.3 流程异常
- 审批中心加载失败显示错误并支持重试。
- 定时任务遇到丢失审批单，标记 `skipped` 并不中断任务。
- 流程执行超时：显示超时提示并支持手动触发。
- 流程状态不一致：提供状态修复机制。

### 8.4 输入异常
- 必填未填、格式不正确、日期不合法时阻断提交并提示。
- 导入预检失败时提供行级错误明细下载。
- 批量操作时部分失败：显示失败原因并提供重试选项。
- 重复提交：防止表单重复提交，显示提交中状态。

### 8.5 系统异常
- 网络异常：显示网络错误提示并支持离线操作（如草稿保存）。
- 浏览器兼容性问题：提示用户使用推荐浏览器。
- 系统崩溃：提供错误日志收集和反馈机制。
...
- 桌面端优先，关键页面支持基础响应式。
- 分辨率支持：1024x768 及以上分辨率。
- 支持 Windows、macOS、Linux 等主流操作系统。

### 9.2 性能
- 列表数据采用分页机制，避免一次性渲染过多。
- 图表按需初始化，减少首页首屏压力。
- 页面加载时间：首屏加载 < 3秒，后续页面 < 2秒。
- 数据查询响应时间：简单查询 < 1秒，复杂查询 < 3秒。
- 支持至少 10,000 条人员数据的管理。
...
### 10.1 功能验收
- 人员新增/编辑/删除全链路可用，数据保存正确。
- 转调流程支持提交、审批、立即生效、定时生效，状态流转正确。
- 外部人员支持申请、审批、到期禁用，状态管理正确。
- 条线节点与成员排序可保存并复显，数据一致性良好。
- 可见性验证返回命中规则与结果，并可查看日志。
- 自定义字段支持添加、编辑、删除，与表单联动正确。
- 展示配置支持实时预览，效果符合预期。
- 编码管理支持导入导出，数据完整性保持。

### 10.2 交互验收
- 关键操作有明确成功/失败反馈，用户体验良好。
- 空态、加载态、错误态完备，视觉反馈清晰。
- 高风险操作有二次确认，防止误操作。
- 表单校验实时反馈，引导用户正确输入。
- 页面跳转流畅，无卡顿现象。
...
### 10.3 权限验收
- 管理角色可操作管理功能，权限控制有效。
- 普通角色不可访问或不可操作受限功能，权限边界清晰。
- 权限变更后，相关功能立即生效，无需刷新页面。

### 10.4 数据一致性验收
- 转调生效后人员主数据与审批状态保持一致，无数据冲突。
- 定时任务执行后队列状态与审批日志一致，状态同步正确。
- 外部人员到期检查后状态更新正确，无遗漏。
- 条线通讯录成员变更后，相关展示同步更新。

### 10.5 性能验收
- 页面加载时间符合要求，首屏加载 < 3秒。
- 数据查询响应时间符合要求，复杂查询 < 3秒。
- 批量操作（如导入1000条数据）执行时间 < 30秒。
- 系统在10,000条人员数据下运行流畅，无明显卡顿。

### 10.6 安全验收
- 敏感字段在列表页脱敏显示，详情页按需展示。
- 操作日志记录完整，关键动作可追溯。
- 权限控制有效，无越权访问现象。
- 表单提交防重复，数据传输加密。
...
### 11.2 已知限制
- Real Provider 尚未实现接口逻辑，当前使用 Mock 数据。
- 部分"编辑/查看"动作仍为占位实现，需后续完善。
- 缺少生产级审计、报警与权限后端校验。
- 移动端应用尚未开发，仅提供演示页面。
- 大规模数据性能优化尚未开展，建议数据量控制在10,000条以内。

### 11.3 后续规划
- 接入真实后端接口，替换 Mock 数据。
- 开发移动端应用，实现移动端通讯录功能。
- 增加数据分析与报表功能，提供组织人员数据洞察。
- 优化大规模数据性能，支持更大规模的组织管理。
- 增加更多高级规则配置，满足复杂权限场景。
...
| `staffing` | string | 否 | `行政编制` | 人员编制 |
| `rank` | string | 否 | `一级调研员` | 职级 |
| `politicalStatus` | string | 否 | `中共党员` | 政治面貌 |
| `entryDate` | string | 否 | `2020-01-01` | 入职时间 |
| `firstWorkDate` | string | 否 | `2015-07-01` | 首次工作时间 |
| `homeAddress` | string | 否 | `北京市朝阳区` | 家庭地址 |
| `shortNumber` | string | 否 | `8888` | 短号 |
| `idCard` | string | 否 | `110101199001011234` | 身份证号 |
...
| `fax` | string | 否 | `0971-xxxxxxx` | 职位传真 |
| `startDate` | string | 是 | `2026-01-01` | 开始日期 |
| `endDate` | string | 否 | `2026-12-31` | 结束日期，空表示至今 |
...
- 条线节点删除时，需先处理子节点和关联成员。
- 自定义字段删除时，需检查是否被其他配置引用。
...
#### A. 组织与人员
- `org_tree_node_click`
  - 参数：`node_id`、`node_level`、`node_name`
- `person_filter_change`
  - 参数：`member_range`、`status`、`filter_count`
- `person_add_submit`
  - 参数：`has_multi_appointment`、`appointment_count`、`field_count`
- `person_edit_submit`
  - 参数：`changed_field_count`、`person_id`
- `person_delete_submit`
  - 参数：`delete_mode`（single/batch）、`person_count`
- `person_import_start` / `person_import_finish`
  - 参数：`strategy`、`file_type`、`success_count`、`fail_count`、`total_count`
- `person_export_submit`
  - 参数：`range`、`field_count`、`person_count`
- `person_detail_view`
  - 参数：`person_id`、`view_duration`

#### B. 转调与审批
- `person_transfer_open`
  - 参数：`from_page`、`person_id`
- `person_transfer_submit`
  - 参数：`effective_mode`、`has_effective_date`、`from_dept`、`to_dept`
- `approval_action_submit`
  - 参数：`action`（pass/reject）、`request_status_before`、`request_id`
- `approval_action_result`
  - 参数：`action`、`request_status_after`、`request_id`
- `transfer_schedule_run`
  - 参数：`processed_count`、`skip_count`、`fail_count`
- `transfer_apply_success`
  - 参数：`person_id`、`from_dept`、`to_dept`、`effective_mode`

#### C. 外部人员
- `external_person_apply_submit`
  - 参数：`org_id`、`data_scope_count`、`expiry_date`、`role`
- `external_person_approve_action`
  - 参数：`action`、`person_id`、`request_id`
- `external_expiry_check_run`
  - 参数：`expired_count`、`disabled_count`、`total_count`
- `external_person_view`
  - 参数：`person_id`、`view_duration`

#### D. 条线与区划
- `line_node_add_submit`
  - 参数：`node_type`、`parent_node_id`、`node_name`
- `line_node_delete_submit`
  - 参数：`node_id`、`children_count`、`member_count`
- `line_member_sort_submit`
  - 参数：`node_id`、`sort_mode`、`changed_count`
- `line_import_finish` / `line_export_submit`
  - 参数：`target_type`（area/line）、`total`、`success_count`、`fail_count`
- `line_contact_view`
  - 参数：`node_id`、`view_duration`

#### E. 可见性与展示配置
- `visibility_rule_save`
  - 参数：`rule_type`（default/custom/advanced）、`permission_type`、`rule_count`
- `visibility_rule_delete`
  - 参数：`rule_id`、`rule_type`
- `visibility_verify_submit`
  - 参数：`scene`、`viewer_role`、`verify_result`、`hit_rule_count`
- `display_config_save`
  - 参数：`section`（contact/user/card）、`changed_count`
- `card_group_reorder`
  - 参数：`group_count`、`from_index`、`to_index`
- `visibility_rule_view`
  - 参数：`rule_type`、`view_duration`

#### F. 自定义字段
- `custom_field_add_submit`
  - 参数：`field_type`、`is_required`
- `custom_field_edit_submit`
  - 参数：`field_id`、`changed_count`
- `custom_field_delete_submit`
  - 参数：`field_id`、`field_name`
- `custom_field_list_view`
  - 参数：`field_count`、`view_duration`

#### G. 系统与通用
- `page_view`
  - 参数：`page_path`、`view_duration`、`referrer`
- `error_occurred`
  - 参数：`error_type`、`error_message`、`page_path`
- `login_attempt`
  - 参数：`login_type`、`result`、`error_code`
- `logout`
  - 参数：`logout_reason`
...
3. 审批通过 `approval_action_result` (action=pass)
4. 执行生效 `transfer_schedule_run`

#### 13.4.2 外部人员接入漏斗
1. 提交申请 `external_person_apply_submit`
2. 审批通过 `external_person_approve_action` (action=pass)
3. 到期检查 `external_expiry_check_run`

#### 13.4.3 规则配置漏斗
1. 打开规则配置页 `visibility_rule_view`
2. 保存规则 `visibility_rule_save`
3. 验证规则 `visibility_verify_submit`

### 13.5 数据质量与分析建议
- 建立埋点数据质量监控，确保事件完整性和准确性。
- 定期分析关键指标，如转调流程成功率、规则验证通过率、导入导出成功率等。
- 针对异常数据进行根因分析，优化系统性能和用户体验。
- 结合业务场景，分析用户行为模式，为产品迭代提供数据支撑。
